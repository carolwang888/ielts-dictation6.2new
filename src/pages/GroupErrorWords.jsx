...
import { ArrowLeft, AlertCircle, Filter, Play, CheckSquare, Square, Minus, Plus, Volume2 } from 'lucide-react';

export default function GroupErrorWords() {
  const { id } = useParams();
  const { getGroup, setErrorCount, initialized, chapters } = useWords();
  ...
  const errorWords = useMemo(() => {
    if (!group) return [];
    return group.words
      .filter(w => w.errorCount >= minErrors && w.errorCount <= maxErrors)
      .map(w => ({ ...w, groupId: group.id, groupName: group.name }))
  ...
  const handleDictateSelected = () => {
    ...
    localStorage.setItem('error-dictation-source-group', id);
    ...
    <div className="space-y-4">
      ...
      {errorWords.length > 0 ? (
        ...
        <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
          ...
          <p className="text-gray-500">本组没有错题！</p>
